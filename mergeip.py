...
fout.write("country,state,city,latitude,longitude,ip,mask\n")
with open('dbip-city-2016-08.csv') as fin:
    ...
    for row in spamreader:
        ...
        try:
            fout.write("\"%s\",\"%s\",\"%s\",\"%s\",\"%s\",\"%s\",\"%s\"\n" % (row[2],row[3],row[4],latitude,longitude,row[0],row[1]))
        except:
            fout.write("\"%s\",\"%s\",\"%s\",\"%s\",\"%s\",\"%s\",\"%s\"\n" % (row[2],row[3],row[4],'None','None',row[0],row[1]))
fout.close()